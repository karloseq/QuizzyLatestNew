...
function StudyCalendar(props) {
  ...
  var recallDates = props.recallDates
  var psd = props.psd 
  ...
  const getBellIcon = (d, newIndex) => {
    ...
    const isStart = currDate.getTime() === new Date(psd.test.started_studying_date).getTime();

    const isEnd = currDate.getTime() === new Date(psd.test.test_date).getTime();

    const isRecall = recallDates.some(
              ...
              <>
                {newRowIndex.includes(index) && (
                  <tr
                    cellspacing="0"
                    cellpadding="0"
                    className={  todayIndex < newRowIndex[newRowIndex.indexOf(index)] && todayIndex > newRowIndex[newRowIndex.indexOf(index-7)]  ? "current" : null
